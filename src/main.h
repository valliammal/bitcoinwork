...
extern int ReadWriteAuxPow(Stream& s, const boost::shared_ptr<CAuxPow>& auxpow, int nType, int nVersion, CSerActionSerialize ser_action); 
template <typename Stream>
extern int ReadWriteAuxPow(Stream& s, boost::shared_ptr<CAuxPow>& auxpow, int nType, int nVersion, CSerActionUnserialize ser_action);
template <typename Stream>
extern int ReadWriteAuxPow(Stream& s, boost::shared_ptr<CAuxPow> const&, int nType, int nVersion, CSerActionGetSerializeSize ser_action);
...
public:
	...
	if (nVersion & BLOCK_VERSION_AUXPOW)
    	{
	    nSerSize += (SerReadWrite(s, FLATDATA(auxpow), nType, nVersion, ser_action));;
    	}
    ...
    bool CheckProofOfWork(uint256, unsigned int) const;
